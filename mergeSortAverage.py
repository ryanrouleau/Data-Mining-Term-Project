...
def main():
    ...
    fIn_matrix = fIn_matrix[1:] # removing column headers from array
    ...
    for row in fIn_matrix:
        ...
        if row[5] != -1: # if there was a matching severity to ICUR in mapping file, otherwise we ignore it
            if prevRowVals == currRowVals: # we're in same block as previous iteration
                blockSeveritySum += row[5] # adding severity
